...
const OrderHistory = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-slate-800 relative pb-20">
      <motion.div
        className="absolute top-0 left-0 w-[500px] h-[500px] bg-emerald-100/50 rounded-full mix-blend-multiply filter blur-3xl opacity-50 pointer-events-none"
        animate={{ y: [0, -15, 0] }}
        transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-orange-100/50 rounded-full mix-blend-multiply filter blur-3xl opacity-50 pointer-events-none"
        animate={{ y: [0, 15, 0] }}
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10">
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-6">
            {orders.map((order, i) => (
              <ScrollReveal key={order.id} variant="fadeUp" delay={0.1 * (i + 1)}>
                <motion.div
                  whileHover={{ y: -4 }}
                  className="glass rounded-[2rem] p-8 border border-white shadow-lg transition-transform"
                >
                  ...
                  <div className="space-y-4 mb-6">
                    {order.items.map((item, index) => (
                      <motion.div
                        key={index}
                        whileHover={{ x: 4 }}
                        className="flex justify-between items-center py-3 px-4 bg-white/60 rounded-xl border border-slate-100"
                      >
